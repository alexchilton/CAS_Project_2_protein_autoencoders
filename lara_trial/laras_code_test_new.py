...
print("\nMolecule DataFrame info:")
print(analyzer.molecule_df.info())
print("\nSample of molecule data:")
print(analyzer.molecule_df.head())

df = analyzer.prepare_autoencoder_input()
print("\nFinal feature columns:")
print(df.columns.tolist())

# Get enhanced features
print("\nGetting enhanced features...")
enhanced_features = analyzer.prepare_autoencoder_input()
print("\nFeature columns:", enhanced_features.columns.tolist())
print("\nShape:", enhanced_features.shape)

# Create graph
#print("\nCreating graph...")
#graph, label_encoder = analyzer.create_graph_with_molecules()
#print("Graph created with", graph.num_nodes, "nodes and", graph.num_edges, "edges")
...
for pdb_file in pdb_files:
    ...
    analyzer = ProteinAnalyzer3(pdb_file, aa_info_file)
    df = analyzer.prepare_autoencoder_input()
    molecules = set(mol.split('_')[0] for mol in df.columns if '_count' in mol)
    dataset.add_protein(df, pdb_file, molecules)

# Get information about all proteins
dataset.get_protein_info()

# Find proteins with specific molecules
proteins_with_hoh = dataset.get_proteins_with_molecule('HOH')
print(f"\nProteins with HOH: {len(proteins_with_hoh)}")

# Find common molecules across all proteins
common_mols = dataset.get_common_molecules()
print(f"\nMolecules present in all proteins: {common_mols}")